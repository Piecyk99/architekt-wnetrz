# Dostawcy — mapa "co skąd"

Pełna mapa zaopatrzenia projektu mieszkania. Korner (płyty, korner.pl) = dostawca formatek default. Reszta = dobierz wg poniższej tabeli.

> **UWAGA — istnieją DWIE niepowiązane firmy o nazwie „Korner" i obie występują w projektach użytkownika.** Samo „Korner" bez doprecyzowania jest ZAKAZANE. Zawsze pisz „Korner (płyty, korner.pl)" albo „Korner (podłogi, korner.eu)". Przypisanie kategorii:
> - docinanie formatek, płyty, blaty, fronty, akcesoria meblowe → **zawsze korner.pl** (oddział Piekary Śląskie / KornerGo)
> - podłogi SPC, listwy przypodłogowe, obrzeża, profile blatowe, cokoły → **korner.eu**

---

## Zasada projektowa

Nie wszystko z jednego sklepu. Optymalizacja jakość/cena wymaga **multi-vendor**: drogie tam gdzie widać, tanie tam gdzie nie widać.

| Kategoria              | Gdzie kupować                                          |
|------------------------|--------------------------------------------------------|
| Meble na wymiar        | **Korner (płyty, korner.pl)** (default) → pracownie lokalne (alt) |
| Płyty / fronty / okucia| Korner (płyty, korner.pl) — oddział Piekary Śląskie / KornerGo |
| Podłogi SPC, listwy przypodłogowe, obrzeża, profile blatowe, cokoły | **Korner (podłogi, korner.eu)** |
| Wykończenia masowe (farba, listwy, klej) | **Leroy Merlin / Castorama**         |
| Glazura, terakota, gres| **Tubądzin, Cersanit, Leroy**                          |
| Mikrocement materiał   | **Festfloor / Topciment** (z wykonawcą)                |
| AGD                    | **Media Markt / RTV Euro AGD / x-kom / Bosch.pl**      |
| Armatura łazienkowa    | **Hansgrohe / Grohe / Geberit** (premium), **Deante / Roca** (średnia) |
| Meble wolnostojące tanie| **IKEA**                                              |
| Meble wolnostojące średnie| **Westwing, Bonami, JYSK** (czasem)                |
| Meble wolnostojące designerskie| **Hay, Muuto, Fritz Hansen** (PL dystrybutorzy) |
| Oświetlenie designerskie| **GoodForm, Sklep Flos, Bujnie**                       |
| Oświetlenie ekonomia   | **Castorama, IKEA, Allegro**                           |
| Dekoracje, tekstylia   | **Westwing, Bonami, H&M Home, Zara Home**              |
| Obrazy / grafika       | **Desenio, Juniqe, Empik (plakaty), Etsy**             |
| Lustra                 | **IKEA, Bonami, Etsy, wytwórnie lokalne**              |
| Rośliny                | **Kwietnik.eu, lokalne kwiaciarnie, IKEA**             |
| Drobnica elektryczna   | **Allegro, Castorama, Bricoman**                       |
| Drzwi wewnętrzne       | **Porta, Pol-Skone, Vasco** (kupować u dystrybutora regionalnego) |
| Profile LED + taśmy    | **Topmet, LedHouse, Klus, Paulmann**                   |
| Włączniki / gniazdka   | **Berker, Schneider Asfora, Kontakt-Simon** (Bricoman, Onninen) |
| Tapeta                 | **Pixers (PL), Cole & Son (UK), Mr Perswall (SE)**     |

---

## Korner (płyty, korner.pl) — dostawca formatek — szczegółowo

Pełna referencja: `korner-katalog.md`. Skrót:

- **Korner Sp. z o.o.** — centrala Strzałków (97-500 Radomsko), infolinia 801 080 550, sprzedaz@korner.pl
- **Oddział domyślny: Piekary Śląskie** (~20 km od Zabrza); alternatywy: Zawiercie, Częstochowa; showroom Katowice
- **Płyty u Korner (korner.pl):** Kronospan, Sonae Arauco, Pfleiderer (KornerGo), kolekcje własne Korner (+ blaty, MDF, HDF, sklejki)
- **Poza ofertą Korner:** Egger, Kronopol, Swiss Krono — realni producenci płyt; jeśli dekor dostępny tylko u nich, wymaga innego dostawcy `[DO WERYFIKACJI]`
- **Fronty** (Maxgloss, Velvet Velour Supermat)
- **Okucia u Korner:** Blum, GTV, Nomet, Häfele, FGV, Peka, Rejs, Titus (pełna lista partnerów w `korner-katalog.md`)
- **Hettich** (w tym Sensys jako tańszy ekwiwalent zawiasów Blum) — niepotwierdzony u Korner, sprawdzić dostępność lub zaopatrzyć się w innym kanale
- **Usługi:** formatowanie, oklejanie krawędzi, CNC, transport
- **Online:** KornerGo (kornergo.pl) + e-Rozkrój (b2b.kornergo.pl)
- **Brak montażu u klienta** (potrzebny wykonawca lokalny)

**Co Korner (płyty) robi:** ostateczne formatki gotowe do skręcenia.
**Co Korner (płyty) nie robi:** instalacja u klienta. Potrzebujesz lokalnego stolarza-montażysty.

### Logistyka formatek — odbiór vs transport

- **Odbiór osobisty (Piekary Śląskie, ~20 km od Zabrza):** opłaca się przy małych i średnich partiach (pojedyncza szafa, dokupienie blend/frontów, drobne korekty) oraz gdy zależy na czasie — bez czekania na okno transportowe. Wymaga auta mieszczącego najdłuższą formatkę; formatki pełnowymiarowe (do 2800 mm) zwykle odpadają.
- **Transport Korner:** opłaca się przy pełnych zabudowach (kuchnia, garderoba) — duża liczba formatek, formatki dłuższe niż mieści auto, ryzyko uszkodzenia krawędzi przy samodzielnym przewozie. Koszt i terminy `[DO WERYFIKACJI]` w oddziale/KornerGo przy wycenie.
- Zamówienie online przez KornerGo / e-Rozkrój z odbiorem w oddziale łączy oba warianty — rozkrój zdalnie, odbiór lokalnie.

---

## Korner (podłogi i profile, korner.eu) — wykończenia, NIE płyty

**Poli-Eco Tworzywa Sztuczne sp. z o.o.**, marka Korner — korner.eu, ul. Zwycięzców 7, 68-200 Żary, tel. 68 478 44 00. **Firma niepowiązana z Korner Sp. z o.o. (korner.pl).**

- **Podłogi SPC:** Solid Floor 5mm, Solid Floor Scandi 5mm, Luxury Floor jodełka 5mm, Natural Floor 4mm, Korner Floor
- **Płytki SPC SPeCtra:** ścienne 4mm, podłogowe 5mm, BIG SIZE 260×122, Select (2026)
- **Podkłady:** SPC Ultra 600+, Ultra Grip 1000, SPC 1200, Performa 3, LVT Protector/Premium, Alu Protect, Thermo/Energy Pro Comfort
- **Listwy przypodłogowe:** Venezia, Evo (Clickfit), XPS Verona, Siena, Quadro, Perfecta, MDF Modena, Ultima/Idea, LP 55/52/50
- **ISTOTNE DLA KUCHNI:** obrzeża meblowe ABS i PVC; profile blatowe LB 15 Mini/ALUmini, LB 23/LB 23 Lira, LB 37, LB 38, LB 32/33/34; uszczelka cokołu LU; system cokołów Alto; uszczelniacz Silikorner
- **Dodatkowo:** panele akustyczne, ścianki działowe, profile aluminiowe do SPC
- **Dystrybucja:** sieć sales outlets. **Brak własnych oddziałów na Śląsku. Brak usługi docinania płyt meblowych.**

**Kontekst projektu użytkownika:** w mieszkaniu położona jest podłoga **Solid Floor 5mm (i4F Drop-Lock)** tej marki — przy doborze listew, progów i uzupełnień podłogi wychodź od tego produktu.

---

## Usługowy rozkrój / oklejanie / WIERCENIE — okolice Zabrza (research www 2026-08-12)

Kontekst: Korner (płyty, korner.pl) tnie i okleja, ale wg inwestora NIE wierci otworów montażowych. Alternatywy lokalne (wszystko `[DO POTWIERDZENIA telefonicznie]` — zakres, ceny, obsługa detaliczna):

| Firma | Adres | Odległość od Zabrza (~) | Usługi wg strony | Kontakt |
|---|---|---|---|---|
| **MEBsystem** | Gliwice, ul. Pszczyńska 206 | ~8 km | cięcie, oklejanie i **wiercenie płyt na maszynach CNC** | mebsystem.pl |
| **Daedalus** | Ruda Śląska, ul. Magazynowa 50 | ~10 km | cięcie, oklejanie (obrzeża 1/2/42), frezowanie, zacinki blatów, ekspres; wiercenie `[?]` | 575 886 996 / 790 886 996, biuro@daedalus.com.pl, pn–pt 8–16 |
| **Soma** | Chorzów, ul. Katowicka 160B (centrala Tychy, Podleska 2) | ~15 km | cięcie, oklejanie (też krzywoliniowe), **wiercenie otworów i frezowanie CNC** | 32 249 76 90, chorzow@soma.tychy.pl; dział CNC (Tychy): 664 011 119 |
| **Komandor Śląsk** | Katowice, ul. Transportowców 35 | ~25 km | cięcie, oklejanie PUR/AirTec, **CNC Homag: otwory pod zawiasy i złącza meblowe**, dostawa 24 h na Śląsku | hkslask.komandor.pl |

Scenariusze: (a) całość (płyty+cięcie+oklejanie+wiercenie) w jednej z powyższych zamiast Korner — porównać cenę płyty; (b) rozkrój w Korner (korner.pl) + samo wiercenie usługowo (dowóz formatek własny). OBI/Castorama: tylko proste cięcie i otwory w blatach — nie do korpusów.

## Leroy Merlin / Castorama — wykończenia masowe

Co tam kupić:
- **Farby** (Dulux, Śnieżka, Beckers, Tikkurila — ale lepiej w salonie Tikkurili)
- **Listwy przypodłogowe** (MDF primer 80-100mm)
- **Klej do paneli / spieku** (Sopro, Mapei)
- **Płyty kartonowo-gipsowe + profile + wkręty**
- **Akcesoria narzędziowe** (sznurek murarski, poziomice, taśmy)
- **Podstawowe oświetlenie funkcjonalne** (halogenki LED, taśmy podstawowe)
- **Sanitarne ekonomiczne** (zawory, syfony, akcesoria)
- **Listwy aluminiowe progowe**

Czego NIE kupować tam:
- Mebli do trzymania (kiepska jakość)
- Oświetlenia hero (mało designerskie)
- Drogiej armatury (lepiej u dystrybutora)
- Płytek lasterskich (lepiej u Tubądzina)

---

## AGD — gdzie kupić

| Marka              | Klasa             | Charakter                       |
|--------------------|--------------------|----------------------------------|
| **Bosch / Siemens**| Premium standard  | Default dla większości, niezawodne |
| **Miele**          | Top premium       | Trwałość 20+ lat, drogie         |
| **Gaggenau**       | Ultra premium     | Designerska zabudowa, $$$$       |
| **Asko**           | Premium skandi    | Cicha, dobre opinie              |
| **Liebherr**       | Lodówki premium   | Chłodzenie najlepsze             |
| **Whirlpool / Amica / Indesit / Beko** | Średnia/ekonomia | Sensowne na 5-10 lat   |
| **Samsung / LG**   | Smart / średnia premium | Dobre lodówki, średnie zmywarki |

### Gdzie kupić AGD

- **RTV Euro AGD** — fizyczne sklepy + dostawa, wymiana stara → nowa
- **Media Markt** — podobne
- **x-kom AGD** — dobre ceny online
- **Bosch.pl / Siemens.pl** — bezpośrednio (czasem promocje + długa gwarancja)
- **Allegro Smart** — najtańsze ale uważaj na "szare" wprowadzane (gwarancja krótsza)

### Wymiary do zaprojektowania

(pełne w `standardy-meble.md` sekcja AGD)

---

## Armatura łazienkowa

### Bateria umywalkowa / wannowa / prysznicowa

| Marka            | Premium                 | Średnia             | Ekonomia                     |
|------------------|--------------------------|---------------------|------------------------------|
| **Grohe**        | Grohe SmartControl       | Grohe Eurosmart     | Grohe BauEdge                |
| **Hansgrohe**    | Hansgrohe Metropol       | Hansgrohe Talis     | Hansgrohe Logis              |
| **KFA / Armatura** | (nie produkują premium) | Linia Smart        | Linia Standard               |
| **Deante**       | Linia Premium            | Linia Easy          | Linia Standard               |

**Default:** Grohe Eurosmart / Hansgrohe Talis (czarny mat) w house style. Premium = SmartControl. Ekonomia = Deante Easy.

### Ceramika sanitarna

| Marka              | Klasa             | Charakter                    |
|--------------------|--------------------|------------------------------|
| **Geberit AquaClean**| Top premium     | WC bidetowe smart            |
| **Villeroy & Boch** | Premium klasyk    | Niezawodna premium niemiecka |
| **Duravit**        | Premium           | Designerska niemiecka        |
| **Laufen**         | Premium szwajcarska| Designerska                  |
| **Roca**           | Średnia            | Hiszpańska, sensowna         |
| **Cersanit**       | Średnia/ekonomia   | Polska, OK                   |
| **Koło**           | Średnia/ekonomia   | Polska, w cenie              |

**Default umywalka:** wpuszczana lub nablatowa Villeroy & Boch / Duravit. Ekonomia = Cersanit.
**Default WC:** podwieszany z Geberit Sigma (spłuczka w ścianie) + miska Villeroy & Boch DirectFlush. Ekonomia = Cersanit DirectFlush.

### Kabiny prysznicowe

| Marka              | Klasa             | Cena (kabina narożna 90×90) |
|--------------------|--------------------|------------------------------|
| **HUPPE**          | Premium niemiecki  | 4000-12000                  |
| **Sanswiss**       | Premium szwajcarski| 4500-10000                  |
| **Sanplast**       | Polski średnia     | 1200-3500                   |
| **Radaway**        | Polski średnia     | 1000-3000                   |
| **Roca**           | Hiszpańska średnia | 1500-4000                   |

---

## IKEA — kiedy używać, kiedy nie

**Używać:**
- Sofy (Vimle, Söderhamn — sprawdzone, modułowe, łatwo rozbudować)
- Krzesła jadalniane (Lisabo, Henriksdal — solidne, długoletnie)
- Tekstylia (zasłony, dywany, poduszki — sprawdzona jakość)
- Pościel (PUDEROST, BOMSKOTTE — bawełniana, dobra cena)
- Drobne meble do sypialni (komody MALM zostawiamy, alternatywa: HEMNES jeśli premium look)
- Akcesoria łazienkowe (ścierki, dozowniki)
- Storage do garderoby (system PAX wewnątrz, ale fronty lepiej Korner (płyty, korner.pl))
- Bazowe oświetlenie (jeśli budżet wymaga)

**Nie używać:**
- Mebli "designerskich" (LANDSKRONA itd. — wyglądają jak imitacja designerskich za 70% mniej, ale jakość 30%)
- Stołów dębowych (klejone elementy, nie litych — Bonami / Westwing lepiej)
- Lamp hero (kompromisowe)

---

## Westwing / Bonami / JYSK

| Sklep         | Charakter                       | Co kupić                          |
|---------------|----------------------------------|-----------------------------------|
| **Westwing**  | Daily deal + nowości             | Stoły, krzesła, kanapy designerskie tańsze, lampy średnia półka |
| **Bonami**    | Stałe ceny, czeska firma         | Stoły dębowe, fotele, dywany, lampy designerskie z rabatami |
| **JYSK**      | Średnia półka skandi             | Łóżka, pościel, krzesła ekonomia  |
| **Miloo Home**| Polska, designerska              | Stoły, krzesła kowalstwo, lampy   |

**Trik:** te same produkty często są w 2-3 z tych sklepów po różnej cenie. Zawsze sprawdź wszystkie.

---

## Designerskie premium — gdzie kupić w Polsce

| Marka            | PL dystrybutor                  | Lokalizacja salonu          |
|------------------|----------------------------------|------------------------------|
| **Hay**          | Hay PL / GoodForm                | Warszawa, Kraków             |
| **Muuto**        | Muuto.pl                         | Warszawa                     |
| **Fritz Hansen** | Fritz Hansen PL                  | Warszawa                     |
| **Vitra**        | Vitra Polska                     | Warszawa                     |
| **&Tradition**   | GoodForm                         | Warszawa, online             |
| **Menu (Audo)**  | GoodForm                         | Warszawa, online             |
| **Ferm Living**  | Pufa Design                      | online, Warszawa             |
| **Northern**     | GoodForm                         | online                       |
| **Carl Hansen**  | Fritz Hansen PL                  | Warszawa                     |
| **Tom Dixon**    | GoodForm                         | online, Warszawa             |
| **Flos**         | Flos Polska                      | Warszawa                     |
| **B&B Italia**   | Drabert                          | Warszawa                     |
| **Cassina**      | Drabert                          | Warszawa                     |

---

## Allegro — kiedy używać

**Używać dla:**
- Drobnicy elektrycznej (kable, koszulki termokurczliwe, gniazdka modułowe)
- Profile LED + zasilacze (czasem 30% taniej niż Castorama)
- Wkręty, kołki, drobne metalowe akcesoria
- Generic akcesoria łazienkowe (haczyki, mydelnice — nie wymagaj marki)
- Niemarkowy mosiądz dekoracyjny (lampki nocne)

**NIE używać dla:**
- "Designerskich" replik (BHP brak, gwarancji brak)
- Armatury (mogą wycieki, BHP fail)
- AGD (mogą szare = brak gwarancji EU)
- Mebli (transport ciężki, ryzyko, brak zwrotu)

---

## Polish projektanci — wsparcie lokalne

Warto wspierać polskich projektantów + producentów (często jakość jak Hay, w cenie 60-70%):

| Marka                | Co produkuje                  | Lokalizacja produkcji |
|----------------------|-------------------------------|------------------------|
| **NAP** (NoLogic)    | Pledy, tekstylia, dywany      | Łódź                   |
| **Tagliato**         | Dywany, tekstylia premium     | Pomorze                |
| **Bumat**            | Lampy mosiądz                 | Warszawa               |
| **Bujnie Studio**    | Lampy industrial              | Warszawa               |
| **Loftlight**        | Lampy mosiądz                 | Warszawa               |
| **Iker**             | Krzesła, stoły                | Mazury                 |
| **Marbet Style**     | Sztukateria poliuretanowa     | Mazowsze               |
| **Mebloform**        | Fotele kowalstwo + tapicerka  | Pomorze                |
| **Vox**              | Zasłony, rolety, panele       | Pomorze                |

---

## Negocjacje cenowe — kiedy działają

| Sklep          | Rabat negocjowalny? | Ile zwykle             |
|----------------|----------------------|------------------------|
| **Korner (płyty, korner.pl)** | TAK   | `[DO WERYFIKACJI]` przy większym zamówieniu |
| **Leroy / Castorama** | NIE             | nie (chyba że promocja czasowa) |
| **IKEA**       | NIE                  | nie                              |
| **Westwing**   | NIE bezpośrednio     | code zniżkowy w newsletterze     |
| **Bonami**     | NIE                  | sale 2-3× w roku                 |
| **Salony designerskie** | TAK         | 5-25%, im więcej tym lepiej      |
| **Dystrybutorzy AGD** (Onninen, Komfort) | TAK | 10-30% przy całej kuchni AGD |
| **Dystrybutorzy armatury** | TAK      | 15-25% przy całej łazience      |

**Trik:** kup wszystko do łazienki (armatura + ceramika + kabina) u **jednego dystrybutora**, a nie w Castoramie / Leroyu. Cena niższa, gwarancja lepsza, doradca zna produkty.

---

## Harmonogram zamawiania

| Tygodnie do oddania | Co zamawiać                                |
|---------------------|--------------------------------------------|
| **T-12 tygodni**    | Drzwi, podłogi, AGD do zabudowy            |
| **T-10**            | Mebel na wymiar — Korner (płyty, korner.pl) |
| **T-8**             | Armatura, ceramika, kabina prysznicowa    |
| **T-6**             | Oświetlenie hero (długi termin importów)  |
| **T-4**             | Wykończenia masowe (farby, listwy)         |
| **T-3**             | Meble wolnostojące (IKEA dostawa szybka)   |
| **T-2**             | Tekstylia, dekoracje                       |
| **T-1**             | Drobnica, rośliny, sprzątanie końcowe      |

Wszystko bardziej niestandardowe (designerskie premium z importu) = **+4-6 tygodni** od standardowego harmonogramu.
